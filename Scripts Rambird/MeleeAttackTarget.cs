...
{ public string TagToAttack;
    ...
    public bool Boss, Flyer, IsLookingAtTheRight, IsLookingAtTheLeft, Persecutioner;
    ...
    { if (Other.gameObject.tag == TagToAttack && Flyer && Persecutioner) { _EnemyPatrolMovement.enabled = false; EnemyRb.velocity = (Other.transform.position - transform.position).normalized * PersuitSpeed; } }

    private void OnTriggerExit2D(Collider2D Other)
    { if (Other.gameObject.tag == TagToAttack) { EnemyRb.velocity = (Other.transform.position - transform.position).normalized * PersuitSpeed; _EnemyPatrolMovement.enabled = true; }
    }
    ...
    void DontCrossTheLimits()
    {
        ...
        private void Update()
        { LastPositionRegistred = EnemyRb.velocity.normalized; UpdateViewOfTheEnemy(); DontCrossTheLimits(); }
     }
